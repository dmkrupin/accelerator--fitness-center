...
/* harmony import */ var _vendor_masking_inputs__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./vendor/masking-inputs */ "./js/vendor/masking-inputs.js");
// Swiper 7.4.1


window.addEventListener('DOMContentLoaded', function () {
  _vendor_masking_inputs__WEBPACK_IMPORTED_MODULE_1__["masking"].init();
});

/***/ }),

/***/ "./js/vendor/masking-inputs.js":
/*!*************************************!*\
  !*** ./js/vendor/masking-inputs.js ***!
  \*************************************/
/*! exports provided: masking */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
...
var masking = {
  ...
  setUpMasks: function setUpMasks(inputs) {
    for (var _i = 0; _i < inputs.length; _i++) {
      masking.createShell(inputs[_i]);
    }
  },
  ...
  createShell: function createShell(input) {
    ...
    input.setAttribute('maxlength', placeholder.length); // input.setAttribute('data-placeholder', placeholder);
    // input.removeAttribute('placeholder');

    text = "<span class=\"shell\">\n      <span aria-hidden=\"true\" id=\"".concat(input.id, "Mask\">\n      <i></i>").concat(placeholder, "</span>\n      ").concat(input.outerHTML, "\n      </span>");
    input.outerHTML = text;
  },
  // Вешаем на каждый из инпутов обработчик отпускания клавиши
  activateMasking: function activateMasking(inputs) {
    for (var _i2 = 0; _i2 < inputs.length; _i2++) {
      masking.maskedInputs[_i2].addEventListener('keyup', function (e) {
        masking.handleValueChange(e);
      }, false);
    }
  },
  // Обновляем значение маски после ввода символа. Добавил формат +7 ...
  setValueOfMask: function setValueOfMask(e) {
    var value = e.target.value;
    var placeholder = e.target.getAttribute('data-placeholder');
    return "+7 <i>".concat(value, "</i>").concat(placeholder.substr(3 + value.length));
  },
  ...
  handleValueChange: function handleValueChange(e) {
    // Получаем id инпута
    var id = e.target.getAttribute('id'); // Пропускаем служебные клавиши
    ...
    document.getElementById(id).value = masking.handleCurrentValue(e); // Записываем в скрытую динамическую маску
  ...
  handleCurrentValue: function handleCurrentValue(e) {
    ...
    var placeholder = isCharsetPresent || e.target.getAttribute('data-placeholder');
    var value = e.target.value;
    var newValue = ''; // Обрезаем спецсимволы и +7

    var strippedValueBuffer = value.replace("+7", ""); // Если 7 - первый символ, то он обрезается, надо поправить

    var strippedValue = isCharsetPresent ? strippedValueBuffer.replace(/\W/g, "") : strippedValueBuffer.replace(/\D/g, "");

    for (var _i3 = 0, j = 0; _i3 < placeholder.length; _i3++) {
      var isInt = !isNaN(parseInt(strippedValue[j]));
      var isLetter = strippedValue[j] ? strippedValue[j].match(/[A-Z]/i) : false;
      var matchesNumber = masking.maskedNumber.indexOf(placeholder[_i3]) >= 0;
      var matchesLetter = masking.maskedLetter.indexOf(placeholder[_i3]) >= 0;
  ...
  errorOnKeyEntry: function errorOnKeyEntry() {// Write your own error handling
  }
};
